...
def generate_command(
    ...
    for topic_idx, topic_data in enumerate(topics, 1):
        ...
        if interactive:
            ...
            while True:
                sys.stdout.write("\n  Akzeptieren [a], neu generieren [r], überspringen [s]?\n")
                sys.stdout.flush()
...
_PROMPT_EXAMPLES = {
    "oumultiresponse": """\
  {{
    "name": "Eine präzise, themenspezifische Frage zu {topic}",
    "questiontext": "Eine konkrete Frage zum Thema {topic}, die Fachwissen prüft?",
    "answers": [
      {{"text": "Eine inhaltlich korrekte Antwort", "correct": true}},
      {{"text": "Eine weitere korrekte Antwort (falls zutreffend)", "correct": true}},
      {{"text": "Eine plausible aber falsche Antwort", "correct": false}},
      {{"text": "Eine weitere falsche Antwort", "correct": false}}
    ]
  }}""",
    "coderunner": """\
  {{
...
def _build_prompt(qtype: str, topic: str, num_questions: int) -> str:
    ...
    return (
        f"Du bist ein erfahrener Didaktik-Experte. "
        f"Erstelle genau {num_questions} {label} zum Thema: '{topic}'.\n\n"
        f"REGELN:\n"
        f"- Jede Frage muss einen konkreten, beschreibenden Namen haben (kein 'Eigenschaft 1' o.ä.)\n"
        f"- Jede Frage muss 4 Antwortmöglichkeiten haben (mindestens 1 richtig, mindestens 1 falsch)\n"
        f"- Fragen und Antworten müssen inhaltlich zum Thema passen\n"
        f"- Keine Platzhalter kopieren, nur echten Inhalt generieren\n\n"
        f"Antworte NUR mit diesem JSON (kein Markdown, kein Text davor/danach):\n"
        f'{{"questions": [\n{example}\n]}}'
    )
