...
Gemini wird ueber die offizielle Google Gemini REST API angebunden. Es ist kein zusaetzliches Python-SDK noetig; die vorhandenen Pakete aus `requirements.txt` reichen.

1. API-Key in Google AI Studio erstellen: https://aistudio.google.com/app/apikey
2. Im Projekt eine `.env` Datei anlegen:
   ```env
   GEMINI_API_KEY=dein_key
   ```
3. Kursgenerator mit Gemini starten:
   ```bash
   python generate_content.py "Java" --provider gemini --model gemini-3.5-flash
   ```

Wenn kein Provider explizit angegeben ist, nutzt der Generator Groq bei `GROQ_API_KEY`, sonst Gemini bei `GEMINI_API_KEY`, sonst Ollama.
